	...
	foreach ($args['actions'] as $item) {
		...
		?>>
			...
			if (!empty($item['image'])) {
				...
				?><div class="sc_action_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr__('Image', 'green-planet')?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
			} else {
				$item['icon_type'] = '';
			...
			if (!empty($item['link']) && !empty($item['link_text'])) {
				?><a href="<?php echo esc_url($item['link']); ?>" class="sc_button sc_button_gradient sc_button_size_large"><?php echo esc_html($item['link_text']); ?></a><?php
			}
			if (!empty($item['info'])) {
