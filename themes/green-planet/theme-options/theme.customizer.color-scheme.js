/* global green_planet_color_schemes, green_planet_dependencies, Color */
...
( function( api ) {
	...
	for (var i in green_planet_color_schemes) {
		cssTemplate[i] = wp.template( 'green_planet-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'green_planet-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {
		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+green_planet_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(green_planet_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', green_planet_customizer_update_load_fonts);		
		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', green_planet_customizer_actions);
		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			green_planet_customizer_check_dependencies(jQuery(this));
		});
		// Refresh color controls
		green_planet_customizer_change_color_scheme(api('color_scheme_editor')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		green_planet_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		green_planet_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function green_planet_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			var id = jQuery(this).attr('id');
			if (id == undefined) return;
			id = id.replace('customize-control-', '');
			var depend = false;
			for (var fld in green_planet_dependencies) {
				if (fld == id) {
					depend = green_planet_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('green_planet_options_no_use');
				} else {
					jQuery(this).hide().addClass('green_planet_options_no_use');
				}
			}
		});
	}
	...
	function green_planet_customizer_refresh_preview(obj) {
		...
		if (id == 'color_scheme_editor') {
			green_planet_customizer_change_color_scheme(val);

		} else if (updateCSS) {
			// Check colors in scheme
			var simple = api('color_settings')()=='simple';
			for (opt in green_planet_color_schemes['default'].colors) {
				if (opt == id) {
					// Store new value in the color table
					green_planet_customizer_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							green_planet_customizer_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							green_planet_customizer_update_color_scheme('alter_hover', val);
						}
					}
					processed = true;
					break;
			...
			// Check theme fonts
			if (!processed) {
				for (opt in green_planet_theme_fonts) {
					for (rule in green_planet_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							green_planet_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) green_planet_customizer_update_css();
		}
	...
	// Actions buttons
	function green_planet_customizer_actions(e) {
	...
	// Store new value in the color table
	function green_planet_customizer_update_color_scheme(opt, value) {
		green_planet_color_schemes[api('color_scheme_editor')()].colors[opt] = value;
		api('scheme_storage').set(green_planet_serialize(green_planet_color_schemes))
	}
	
	// Store new value in the theme fonts
	function green_planet_customizer_update_theme_fonts(opt, rule, value) {
		green_planet_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function green_planet_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=green_planet_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in green_planet_theme_fonts) {
	...
	function green_planet_customizer_change_color_scheme(value) {
		updateCSS = false;
		for (var opt in green_planet_color_schemes[value].colors) {
			if (api(opt) == undefined) continue;
			api( opt ).set( green_planet_color_schemes[value].colors[opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', green_planet_color_schemes[value].colors[opt] )
				.wpColorPicker( 'defaultColor', green_planet_color_schemes[value].colors[opt] );
		}
		updateCSS = true;
		green_planet_customizer_update_css();
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function green_planet_customizer_update_css() {
		...
		var fonts = green_planet_customizer_add_theme_fonts(green_planet_theme_fonts);

		// Make styles and add into css
		css += green_planet_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in green_planet_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in green_planet_color_schemes[scheme].colors) {
				colors[i] = green_planet_color_schemes[scheme].colors[i];
			}
            ...
            // Make styles and add into css
            var tmpl = jQuery( '#tmpl-green_planet-color-scheme-' + scheme ).html().trim();
            for (var i in colors) {
	...
	function green_planet_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + green_planet_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + green_planet_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + green_planet_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function green_planet_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function green_planet_customizer_prepare_html_value(val) {
