...
if (!function_exists('green_planet_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'green_planet_hovers_theme_setup3', 3 );
	function green_planet_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		green_planet_storage_set_array_before('options', 'sidebar_widgets', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'green-planet'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'green-planet') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'green-planet'),
					'slide_top'		=> esc_html__('Slide from Top',		'green-planet'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'green-planet'),
				),
				"type" => "hidden"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'green-planet'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'green-planet') ),
				"std" => 'icon',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'green-planet'),
					'icon'	=> esc_html__('Icon',	'green-planet'),
					'icons'	=> esc_html__('Icons',	'green-planet'),
					'zoom'	=> esc_html__('Zoom',	'green-planet'),
					'fade'	=> esc_html__('Fade',	'green-planet'),
					'slide'	=> esc_html__('Slide',	'green-planet'),
					'pull'	=> esc_html__('Pull',	'green-planet'),
					'border'=> esc_html__('Border',	'green-planet')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('green_planet_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'green_planet_hovers_theme_setup9', 9 );
	function green_planet_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'green_planet_hovers_frontend_scripts', 1010 );
		add_filter( 'green_planet_filter_localize_script','green_planet_hovers_localize_script' );
		add_filter( 'green_planet_filter_merge_scripts',	'green_planet_hovers_merge_scripts' );
		add_filter( 'green_planet_filter_merge_styles',	'green_planet_hovers_merge_styles' );
		add_filter( 'green_planet_filter_get_css', 		'green_planet_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'green_planet_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'green_planet_hovers_frontend_scripts', 1010 );
	function green_planet_hovers_frontend_scripts() {
		if ( green_planet_is_on(green_planet_get_theme_option('debug_mode')) && green_planet_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'green_planet-hovers', green_planet_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( green_planet_is_on(green_planet_get_theme_option('debug_mode')) && green_planet_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'green_planet-hovers',  green_planet_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('green_planet_hovers_merge_scripts')) {
	//Handler of the add_filter( 'green_planet_filter_merge_scripts', 'green_planet_hovers_merge_scripts' );
	function green_planet_hovers_merge_scripts($list) {
...
if (!function_exists('green_planet_hovers_merge_styles')) {
	//Handler of the add_filter( 'green_planet_filter_merge_styles', 'green_planet_hovers_merge_styles' );
	function green_planet_hovers_merge_styles($list) {
...
if (!function_exists('green_planet_hovers_localize_script')) {
	//Handler of the add_filter( 'green_planet_filter_localize_script','green_planet_hovers_localize_script' );
	function green_planet_hovers_localize_script($arr) {
		$arr['button_hover'] = green_planet_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('green_planet_hovers_add_icons') ) {
	function green_planet_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink());; ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') esc_html_e('Details', 'green-planet');
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'green_planet_hovers_get_css' ) ) {
	//Handler of the add_filter( 'green_planet_filter_get_css', 'green_planet_hovers_get_css', 10, 4 );
	function green_planet_hovers_get_css($css, $colors, $fonts, $scheme='') {
