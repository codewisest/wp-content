	...
	class TRX_DONATIONS {
		...
		function admin_settings() {
			...
			<div class="trx_donations_options">
				...
				<h3 class="trx_donations_options_title"><?php esc_html_e('Donations settings', 'trx_donations'); ?></h3>
            ...
            $output = '<div ' . ($id ? ' id="'.esc_attr($id).'"' : '')
					...
					. '>'
						. (!empty($subtitle) 
							...
							: '')
						. (!empty($title) 
							? '<h3 class="sc_donations_form_title sc_item_title">' . trim($title) . '</h3>' 
							: '')
						. (!empty($description) 
                ...
                . ( !empty($privacy)
                    ...
                    : ''
						...
						. '<div class="sc_donations_form_field sc_donations_form_field_button"><input type="button" class="sc_button sc_donations_form_submit sc_button_size_large" value="'.esc_html__('Donation', 'trx_donations').'"></div>'
					. '</form>'
				. '</div>';
			...
			if (file_exists($tpl = trx_donations_get_file_dir( 'templates/content-'.$style.'.php' ))) {
				...
				$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
							. ' class="sc_donations'
								...
								. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
							. '>'
							. (!empty($title) ? '<h3 class="sc_donations_title sc_item_title">' . trim($title) . '</h3>' : '')
							...
							. ($columns > 1 
								? '<div class="'.($this->get_option('columns_wrap_class')!='' ? esc_attr($this->get_option('columns_wrap_class')) : 'sc_donations_columns_wrap columns_' . $columns).'">' 
								: '');
